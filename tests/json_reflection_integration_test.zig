...
fn parseJsonValue(comptime T: type, json_value: std.json.Value) !T {
    ...
    switch (info) {
        .bool => {
            if (json_value != .bool) return ToolError.InvalidInput;
            return json_value.bool;
        },
        .int => {
            if (json_value != .integer) return ToolError.InvalidInput;
        ...
        .pointer => |ptr_info| {
            ...
            if (json_value != .string) return ToolError.InvalidInput;
...
fn errorTestTool(allocator: std.mem.Allocator, params: std.json.Value) ToolError!std.json.Value {
    ...
    if (params_obj.get("error_type")) |error_type| {
        if (error_type == .string) {
            if (std.mem.eql(u8, error_type.string, "missing_param")) {
                return ToolError.MissingParameter;
            } else if (std.mem.eql(u8, error_type.string, "invalid_input")) {
                return ToolError.InvalidInput;
            } else if (std.mem.eql(u8, error_type.string, "network_error")) {
...
test "JSON reflection system basic serialization and deserialization" {
    ...
    try json_obj.put("message", std.json.Value{ .string = original.message });
    try json_obj.put("count", std.json.Value{ .integer = original.count });
    try json_obj.put("enabled", std.json.Value{ .bool = original.enabled });
    try json_obj.put("optional_field", std.json.Value{ .string = original.optional_field.? });
...
test "Tool registry integration with new JSON tools" {
    ...
    const tool_fn = registry.get("mock_tool");
    try testing.expect(tool_fn != null);
