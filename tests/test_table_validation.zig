...
test "table validation - detects inconsistent column count" {
    ...
    var found_consistency_issues = false;
    for (result.issues) |issue| {
        if (issue.issue_type == .inconsistent_column_count) {
            found_consistency_issues = true;
            break;
        }
    }
    try expect(found_consistency_issues);
}
...
test "table validation - detects empty cells" {
    ...
    var found_empty_cell_issues = false;
    for (result.issues) |issue| {
        if (issue.issue_type == .empty_cells) {
            found_empty_cell_issues = true;
            break;
        }
    }
    try expect(found_empty_cell_issues);
}
...
test "table validation - detects alignment mismatch" {
    ...
    var found_alignment_issue = false;
    for (result.issues) |issue| {
    ...
    try expect(found_alignment_issue);
}
...
test "table repair - fixes column consistency" {
    ...
    try expectEqual(@as(usize, 3), test_table.headers.len);
    for (test_table.rows) |row| {
        try expectEqual(@as(usize, 3), row.len);
    }

    // Check that missing cell was filled with placeholder
    try expectEqualSlices(u8, "-", test_table.rows[0][2]); // Should be placeholder for missing city
}
...
test "table repair - trims whitespace" {
    ...
    try expectEqualSlices(u8, "Name", test_table.headers[0]);
    try expectEqualSlices(u8, "City", test_table.headers[2]);
    try expectEqualSlices(u8, "John", test_table.rows[0][0]);
    try expectEqualSlices(u8, "25", test_table.rows[0][1]);
    try expectEqualSlices(u8, "NYC", test_table.rows[0][2]);
}
...
test "table repair - fills empty cells" {
    ...
    try expectEqualSlices(u8, "N/A", test_table.rows[0][1]); // Age should be N/A
    try expectEqualSlices(u8, "N/A", test_table.rows[1][0]); // Name should be N/A
    try expectEqualSlices(u8, "N/A", test_table.rows[1][2]); // City should be N/A
}
...
test "table repair - normalizes alignments" {
    ...
    try expectEqual(table.Alignment.left, test_table.alignments[0]);
    try expectEqual(table.Alignment.center, test_table.alignments[1]);
    try expectEqual(table.Alignment.left, test_table.alignments[2]); // Should default to left
}
...
test "table validate and repair combined" {
    ...
    var initial_result = try table.validateTable(allocator, &test_table, validation_config);
    defer initial_result.deinit(allocator);

    try expect(!initial_result.is_valid);
    try expect(initial_result.issues.len > 0);

    // Perform combined validate and repair
    var combined_result = try table.validateAndRepairTable(allocator, &test_table, validation_config, repair_config);
    defer combined_result[0].deinit(allocator);

    const final_validation = combined_result[0];
    const repairs_made = combined_result[1];

    try expect(repairs_made > 0);
    try expect(final_validation.is_valid or final_validation.issues.len < initial_result.issues.len);
    ...
    // Verify empty cells were filled
    try expectEqualSlices(u8, "N/A", test_table.rows[1][0]); // Empty name should be N/A
}
