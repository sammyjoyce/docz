...
test "cursor consolidation - all modules accessible" {
    // Test that we can access cursor types from unified module
...
test "unified cursor controller compiles with proper writer type" {
    var gpa = std.heap.GeneralPurposeAllocator(.{}){};
    defer _ = gpa.deinit();
    const allocator = gpa.allocator();
    ...
    const ControllerType = cursor.UnifiedCursorControllerFor(@TypeOf(test_writer));
