...
pub const InteractiveMarkdownSession = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, config: InteractiveMarkdownConfig) !*Self {
        ...
        const current_theme = config.ui_config.theme_manager;

        // Initialize input manager
        const input = try Input.init(allocator);

        // Initialize editor
        const editor = try MarkdownEditor.init(allocator, renderer, current_theme);

        // Initialize preview renderer
        const preview_renderer = try PreviewRenderer.init(allocator, renderer, current_theme);
    ...
    fn handleInput(self: *Self) !void {
        if (try self.input.pollEvent()) |event| {
            switch (event) {
                .key_press => |key_event| {
    ...
    fn renderStatusBar(self: *Self) !void {
        ...
        const current_theme = self.theme.getCurrentTheme();

        // Draw status bar background
        const bg_style = Style{
            .bg_color = current_theme.primary,
            .fg_color = current_theme.background,
            .bold = false,
        };
    ...
    fn renderOutline(self: *Self) !void {
        ...
        const current_theme = self.theme.getCurrentTheme();
        const bg_style = Style{
            .bg_color = current_theme.secondary,
...
pub const PreviewRenderer = struct {
    ...
    pub fn render(self: *PreviewRenderer, bounds: Bounds, content: []const u8) !void {
        ...
        // For now, just show the raw markdown with standard formatting
