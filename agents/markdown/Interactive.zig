...
//! - **Multiple preview modes** (text, rich, print)
//!
...
//!
//! ```zig
//! const interactive_editor = @import("Interactive.zig");
//!
...
pub const InteractiveConfig = struct {
    ...
    baseConfig: MarkdownEditorConfig,

    /// Enable live preview with split pane
    enableLivePreview: bool = true,

    /// Preview synchronization settings
    previewSync: PreviewSyncConfig = .{},
...
pub const PreviewSyncConfig = struct {
    ...
    enableScrollSync: bool = true,

    /// Synchronization delay in milliseconds
    syncDelayMs: u32 = 150,

    /// Preview update mode
    updateMode: PreviewUpdateMode = .debounced,

    /// Show preview errors
    showPreviewErrors: bool = true,
};
...
pub const NavigationConfig = struct {
    ...
    enableQuickJump: bool = true,

    /// File tree width
    fileTreeWidth: u16 = 25,
...
pub const VisualToolsConfig = struct {
    ...
    enableTableGenerator: bool = true,

    /// Enable link manager
    enableLinkManager: bool = true,

    /// Enable image preview
    enableImagePreview: bool = true,

    /// Enable code block formatter
    enableCodeFormatter: bool = true,

    /// Enable diff viewer
    enableDiffViewer: bool = true,
...
pub const SessionConfig = struct {
    ...
    autoSaveInterval: u32 = 30,

    /// Show auto-save indicators
    showAutoSaveIndicators: bool = true,
};
...
pub const StatusBarConfig = struct {
    ...
    showDocumentStats: bool = true,

    /// Show markdown lint status
    showLintStatus: bool = true,

    /// Show save status
    showSaveStatus: bool = true,
...
pub const IntegrationConfig = struct {
    ...
    dashboardUpdateMs: u32 = 1000,

    /// Enable external tool integration
    enableExternalTools: bool = false,
};
...
pub const InteractiveMarkdownEditor = struct {
    ...
    pub fn init(
        ...
        const layout = try Layout.init(allocator, &editor_config);
    ...
    fn render(self: *Self) !void {
        ...
        // Calculate layout
        const layout = try self.layout.calculateLayout(size);
    ...
    fn renderLayout(self: *Self, renderer: *tui.Renderer, layout: LayoutDefinition) !void {
        // Render top bar
    ...
    fn setupTerminal(self: *Self) !void {
        _ = self;
        // Terminal setup with mouse support, alternate screen, etc.
    }
    ...
    fn showWelcomeScreen(self: *Self) !void {
        // Welcome screen with recent files, tips, etc.
...
/// Component for integrated widgets
...
/// Layout for managing split panes and responsive layout
pub const Layout = struct {
    ...
    pub fn calculateLayout(self: *Self, size: term.TerminalSize) !LayoutDefinition {
...
pub const ComponentExtensions = struct {
    /// Handle key event
    pub fn handleKeyEvent(self: *Component, key: tui.Key) !bool {
    ...
    /// Handle mouse event
    pub fn handleMouseEvent(self: *Component, mouse: tui.Mouse) !void {
...
pub const MarkdownEditorExtensions = struct {
    pub fn handleKeyEvent(self: *markdown_editor.MarkdownEditor, key: tui.Key) !bool {
        return try self.handleKeyEvent(key);
    }

    /// Handle mouse event
    pub fn handleMouseEvent(self: *markdown_editor.MarkdownEditor, mouse: tui.Mouse) !void {
