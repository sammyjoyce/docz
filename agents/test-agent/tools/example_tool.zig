//! Example tool implementation for test-agent.
...
pub fn execute(allocator: std.mem.Allocator, params: std.json.Value) tools_mod.ToolError!std.json.Value {
    ...
    while (i < options.repeat) : (i += 1) {
        if (i > 0) {
            try result_builder.appendSlice(" ");
        }

        if (options.uppercase) {
            // Convert to uppercase
            for (message) |char| {
                try result_builder.append(std.ascii.toUpper(char));
            }
        } else {
            try result_builder.appendSlice(message);
        }
    }

    // Build JSON response
    var result_obj = std.json.ObjectMap.init(allocator);
    try result_obj.put("result", std.json.Value{ .string = try result_builder.toOwnedSlice() });
