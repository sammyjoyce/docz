...
const input = @import("../src/term/enhanced_input.zig");

/// Demonstration of the enhanced input system
/// Based on charmbracelet/x/input features with Zig implementation
...
fn demoMouseEvents(allocator: std.mem.Allocator) !void {
    ...
    const events = [_]input.InputEvent{
        ...
        input.InputEvent{ .mouse_wheel = input.Mouse{
            .x = 30, .y = 25, .button = .wheel_up, .mod = .{}
        }},
        input.InputEvent{ .mouse_motion = input.Mouse{
            .x = 40, .y = 35, .button = .left, .mod = .{}
        }},
    };
