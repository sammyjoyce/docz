...
pub fn main() !void {
    ...
    // Demo the smart TUI components
    try demoSmartNotifications();
    try demoSmartProgressBars(renderer, terminal_size);
...
fn demoSmartNotifications() !void {
    std.debug.print("🔔 Demo 1: Smart Notifications\n");
    std.debug.print("   Progressive enhancement from basic terminal bell to rich OSC notifications\n\n");
    
    try tui.notifyInfo("Demo Started", "Welcome to the smart TUI demonstration!");
...
fn demoSmartProgressBars(renderer: *tui.Renderer, terminal_size: bounds_mod.TerminalSize) !void {
    std.debug.print("📊 Demo 2: Smart Progress Bars\n");
    std.debug.print("   Multiple visual styles that adapt based on terminal support\n\n");
    
    const progress_styles = [_]tui.smart_progress.SmartProgressBar.ProgressStyle{
        .bar, .blocks, .gradient, .dots, .spinner
    };
    
    const style_names = [_][]const u8{
        "Traditional Bar", "Unicode Blocks", "Color Gradient", "Animated Dots", "Spinner"
    };
    ...
    while (progress <= 1.0) : (progress += 0.15) {
        ...
        for (progress_styles, style_names) |style, name| {
            ...
            var progress_bar = tui.smart_progress.SmartProgressBar.init(name, style);
...
fn demoBoxDrawing(renderer: *tui.Renderer, caps: tui.TermCaps) !void {
    ...
    const box_styles = [_]tui.BoxStyle.BorderStyle.LineStyle{
        .single, .double, .rounded, .thick, .dotted
    };
    
    const box_names = [_][]const u8{
        "Single", "Double", "Rounded", "Thick", "Dotted"
    };
