...
fn testComponentFunctionality(allocator: std.mem.Allocator) !void {
    ...
    const progress_tests = [_]Progress{
        .{ .value = 0.0, .label = "Start", .percentage = true },
        .{ .value = 0.5, .label = "Middle", .percentage = true, .color = Color.ansi(.yellow) },
        .{ .value = 1.0, .label = "Complete", .percentage = true, .color = Color.ansi(.green) },
    };
    ...
    try adaptive_render.renderTable(renderer, test_table);
    std.debug.print("✓\n");
    ...
    for (chart_types) |chart_type| {
        ...
        try adaptive_render.renderChart(renderer, test_chart);
    }
    std.debug.print("✓\n");
...
test "comprehensive adaptive rendering tests" {
    ...
    // Test progress
    const progress = Progress{ .value = 0.5, .label = "Test", .percentage = true };
    try adaptive_render.renderProgress(renderer, progress);
    ...
    var test_renderer = try AdaptiveRenderer.init(testing.allocator);
    defer test_renderer.deinit();

    try adaptive_render.renderProgress(test_renderer, progress);
    try adaptive_render.renderTable(test_renderer, table);
    try adaptive_render.renderChart(test_renderer, chart);
