...
fn runCLIDemo(allocator: Allocator, args: [][]const u8) !void {
    ...
    const exit_code = try cli_instance.run(args);
...
fn runTUIDemo(allocator: Allocator) !void {
    ...
    const main_panel = try demo_widget.DemoPanel.init(allocator, Rect{ .x = 2, .y = 2, .width = terminal_size.width / 2 - 3, .height = terminal_size.height - 6 }, "Enhanced TUI System");
    defer main_panel.deinit();
    try renderer.addWidget(main_panel.asWidget());

    // Side panel (right side)
    const side_panel = try demo_widget.DemoPanel.init(allocator, Rect{ .x = @as(i16, @intCast(terminal_size.width / 2 + 1)), .y = 2, .width = terminal_size.width / 2 - 3, .height = terminal_size.height - 10 }, "Terminal Features");
    ...
    // Button panel (bottom)
    const quit_button = try demo_widget.Button.init(allocator, Rect{ .x = @as(i16, @intCast(terminal_size.width / 2 - 5)), .y = @as(i16, @intCast(terminal_size.height - 4)), .width = 10, .height = 3 }, "Quit");
