...
//! Fully leverages src/term capabilities for rich visualizations

const std = @import("std");
const term_mod = @import("../../../src/term");
...
pub const AdaptiveDashboard = struct {
    ...
    fn renderHeader(self: *Self) !void {
        ...
        const title_text = try std.fmt.allocPrint(
            self.allocator,
            "{s} - {s}",
            .{ self.config.title, self.getCapabilitiesSummary() }
        );
        defer self.allocator.free(title_text);
        
        try self.renderer.drawText(
            .{ .x = header_bounds.x + 2, .y = header_bounds.y + 1 },
            title_text,
        ...
        const stats_text = try std.fmt.allocPrint(
            self.allocator,
            "Frame: {} | Updated: {}s ago",
            .{ self.frame_count, std.time.timestamp() - self.last_update }
        );
        defer self.allocator.free(stats_text);
        
        const stats_x = header_bounds.x + header_bounds.width - stats_text.len - 2;
        try self.renderer.drawText(
            .{ .x = @intCast(stats_x), .y = header_bounds.y + 1 },
            stats_text,
            self.theme.stats_style
        );
    }
    ...
    fn renderFooter(self: *Self) !void {
        ...
        try self.renderer.drawText(
            .{ .x = footer_bounds.x + 2, .y = footer_bounds.y },
            shortcuts,
