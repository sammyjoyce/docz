...
pub const Dashboard = struct {
    ...
    fn renderTokenUsage(self: *Self, bounds: Bounds) !void {
        ...
        try self.renderer.writeText(bounds.x + 14, y_offset, try std.fmt.allocPrint(self.allocator, "{d}", .{self.stats.total_tokens}));

        try self.renderer.writeText(bounds.x, y_offset + 2, "Recent Usage:");
        try self.renderer.writeText(bounds.x + 2, y_offset + 3, try std.fmt.allocPrint(self.allocator, "Input:  {d}", .{total_in}));
        try self.renderer.writeText(bounds.x + 2, y_offset + 4, try std.fmt.allocPrint(self.allocator, "Output: {d}", .{total_out}));
    ...
    fn renderConversationHistory(self: *Self, bounds: Bounds) !void {
        ...
        for (self.conversation_history.items[start_index..]) |entry| {
            ...
            if (entry.api_metrics) |metrics| {
                const info_str = try std.fmt.allocPrint(self.allocator, "Tokens: {d} | ${d:.4}", .{ metrics.tokens_in + metrics.tokens_out, metrics.cost_usd });
    ...
    fn renderSystemResources(self: *Self, bounds: Bounds) !void {
        ...
        try self.renderHorizontalBar(bounds.x + 5, y_offset, bounds.width - 10, latest.cpu_usage_percent / 100.0);
        try self.renderer.writeText(bounds.x + bounds.width - 5, y_offset, try std.fmt.allocPrint(self.allocator, "{d:.1}%", .{latest.cpu_usage_percent}));
        ...
        try self.renderHorizontalBar(bounds.x + 5, y_offset + 2, bounds.width - 10, mem_percent);
        try self.renderer.writeText(bounds.x + bounds.width - 8, y_offset + 2, try std.fmt.allocPrint(self.allocator, "{d:.0}MB", .{latest.memory_usage_mb}));

        // Network I/O
        if (bounds.height >= 6) {
            const net_str = try std.fmt.allocPrint(self.allocator, "NET: ↓{d} ↑{d} KB/s", .{
        ...
        // Disk I/O
        if (bounds.height >= 8) {
            const disk_str = try std.fmt.allocPrint(self.allocator, "DISK: R:{d} W:{d} KB/s", .{
    ...
    fn renderToolAnalytics(self: *Self, bounds: Bounds) !void {
        ...
        try self.renderer.writeText(bounds.x, y_offset, try std.fmt.allocPrint(self.allocator, "Total Executions: {d}", .{self.stats.total_tool_executions}));

        try self.renderer.writeText(bounds.x, y_offset + 2, try std.fmt.allocPrint(self.allocator, "Avg Time: {d:.2}ms", .{self.stats.avg_tool_execution_time_ms}));
        ...
        if (bounds.height >= 6) {
            ...
            while (iter.next()) |entry| : (row += 1) {
                ...
                const tool_str = try std.fmt.allocPrint(self.allocator, "  {s}: {d}", .{ entry.key_ptr.*, entry.value_ptr.* });
    ...
    fn renderStatusOverview(self: *Self, bounds: Bounds) !void {
        ...
        const success_rate = if (self.stats.total_api_calls > 0)
            ...
            100.0;
        try self.renderer.writeText(bounds.x + 14, y_offset + 2, try std.fmt.allocPrint(self.allocator, "{d:.1}%", .{success_rate}));
    }
    ...
    fn renderApiLatency(self: *Self, bounds: Bounds) !void {
        ...
        try self.renderer.writeText(bounds.x, y_offset + 1, try std.fmt.allocPrint(self.allocator, "  P50: {d:.0}", .{latencies[p50_index]}));
        try self.renderer.writeText(bounds.x, y_offset + 2, try std.fmt.allocPrint(self.allocator, "  P95: {d:.0}", .{latencies[p95_index]}));
        try self.renderer.writeText(bounds.x, y_offset + 3, try std.fmt.allocPrint(self.allocator, "  P99: {d:.0}", .{latencies[p99_index]}));
    }
    ...
    fn renderThroughput(self: *Self, bounds: Bounds) !void {
        ...
        try self.renderer.writeText(bounds.x, y_offset + 1, try std.fmt.allocPrint(self.allocator, "  {d} req/min", .{requests_last_minute}));
        ...
        if (bounds.height >= 4) {
            ...
            try self.renderer.writeText(bounds.x, y_offset + 3, try std.fmt.allocPrint(self.allocator, "  {d:.1} tok/s", .{tokens_per_sec}));
        }
    }
    ...
    fn renderAlerts(self: *Self) !void {
        ...
        for (self.alerts.items, 0..) |alert, i| {
            ...
            const alert_str = try std.fmt.allocPrint(self.allocator, "[{s}] {s}", .{ @tagName(alert.severity), alert.message });
    ...
    fn renderStatusBar(self: *Self, x: u16, y: u16, width: u16) !void {
        const status_str = try std.fmt.allocPrint(self.allocator, "API: {d} | Tokens: {d} | Cost: ${d:.4} | Uptime: {s}", .{
    ...
    fn renderRecentActivity(self: *Self, x: u16, y: u16, width: u16, height: u16) !void {
        ...
        for (recent) |api| {
            ...
            const activity_str = try std.fmt.allocPrint(self.allocator, "{s} {s} {s} {d}tok {d}ms", .{ time_str, status, api.model, api.tokens_in + api.tokens_out, @as(u32, @intFromFloat(api.latency_ms)) });
    ...
    fn drawBorder(self: *Self, bounds: Bounds, title: []const u8) !void {
        ...
        if (title.len > 0 and bounds.width > title.len + 4) {
            ...
            try self.renderer.writeText(title_x, bounds.y, try std.fmt.allocPrint(self.allocator, " {s} ", .{title}));
        }
    ...
    fn renderCostBreakdown(self: *Self, x: u16, y: u16, width: u16) !void {
        ...
        while (iter.next()) |entry| : (row += 1) {
            ...
            const cost_str = try std.fmt.allocPrint(self.allocator, "{s}: ${d:.4}", .{ entry.key_ptr.*, entry.value_ptr.* });
    ...
    fn checkApiAlerts(self: *Self, metrics: ApiCallMetrics) !void {
        ...
        if (metrics.latency_ms > self.config.alert_thresholds.max_response_time_ms) {
            try self.alerts.append(.{
                .type = .high_latency,
                .message = try std.fmt.allocPrint(self.allocator, "High latency: {d:.0}ms", .{metrics.latency_ms}),
                .severity = .warning,
        ...
        if (total_tokens > self.config.alert_thresholds.max_tokens_per_request) {
            try self.alerts.append(.{
                .type = .token_limit,
                .message = try std.fmt.allocPrint(self.allocator, "High token usage: {d}", .{total_tokens}),
                .severity = .warning,
                .timestamp = std.time.milliTimestamp(),
            });
        }
        ...
        if (metrics.cost_usd > self.config.alert_thresholds.max_cost_per_request) {
            try self.alerts.append(.{
                .type = .high_cost,
                .message = try std.fmt.allocPrint(self.allocator, "High cost: ${d:.4}", .{metrics.cost_usd}),
                .severity = .warning,
                .timestamp = std.time.milliTimestamp(),
            });
        }
        ...
        if (self.stats.error_rate_percent > self.config.alert_thresholds.error_rate_threshold) {
            try self.alerts.append(.{
                ...
                .message = try std.fmt.allocPrint(self.allocator, "Error rate: {d:.1}%", .{self.stats.error_rate_percent}),
                .severity = .err,
                .timestamp = std.time.milliTimestamp(),
            });
        }
    ...
    fn checkSystemAlerts(self: *Self, metrics: SystemMetrics) !void {
        ...
        if (metrics.cpu_usage_percent > self.config.alert_thresholds.cpu_threshold_percent) {
            try self.alerts.append(.{
                .type = .high_cpu,
                .message = try std.fmt.allocPrint(self.allocator, "High CPU: {d:.1}%", .{metrics.cpu_usage_percent}),
                .severity = .warning,
                .timestamp = std.time.milliTimestamp(),
            });
        }
        ...
        if (metrics.memory_usage_mb > self.config.alert_thresholds.memory_threshold_mb) {
            try self.alerts.append(.{
                .type = .high_memory,
                .message = try std.fmt.allocPrint(self.allocator, "High memory: {d:.0}MB", .{metrics.memory_usage_mb}),
                .severity = .warning,
                .timestamp = std.time.milliTimestamp(),
            });
        }
