...
const DemoState = struct {
    ...
    fn renderStatusPanel(self: *Self) !void {
        ...
        try self.renderer.drawBorder(
            tui.Bounds{ .x = 2, .y = start_y, .width = 60, .height = 10 }, 
            tui.BoxStyle.rounded
        );
        ...
        try self.renderer.drawText(4, start_y + 2, std.fmt.allocPrint(
            self.allocator, 
            "Current Widget: {d} | Focus: {s} | Mouse: ({d},{d})", 
            .{ self.current_widget + 1, if (self.focus_manager.hasFocus()) "Yes" else "No", self.mouse_position.x, self.mouse_position.y }
        ) catch "Status unavailable");
        
        try self.renderer.drawText(4, start_y + 3, std.fmt.allocPrint(
            self.allocator,
            "Paste Mode: {s} | Terminal: {d}x{d}",
            .{ if (self.paste_manager.isPasting()) "Active" else "Inactive", self.terminal_size.width, self.terminal_size.height }
        ) catch "Status unavailable");
        
        // Widget content preview
        try self.renderer.drawText(4, start_y + 5, "Widget Contents:");
        try self.renderer.drawText(4, start_y + 6, std.fmt.allocPrint(
            self.allocator,
            "1: {s}",
            .{if (self.text_input1.getText().len > 0) self.text_input1.getText()[0..@min(30, self.text_input1.getText().len)] else "(empty)"}
        ) catch "1: (error)");
        try self.renderer.drawText(4, start_y + 7, std.fmt.allocPrint(
            self.allocator,
            "2: {s}",
            .{if (self.text_input2.getText().len > 0) self.text_input2.getText()[0..@min(30, self.text_input2.getText().len)] else "(empty)"}
        ) catch "2: (error)");
        try self.renderer.drawText(4, start_y + 8, std.fmt.allocPrint(
            self.allocator,
            "3: {s}",
            .{if (!self.password_input.isEmpty()) "***password***" else "(empty)"}
        ) catch "3: (error)");
    }
    ...
    fn renderEventLog(self: *Self) !void {
        ...
        try self.renderer.drawBorder(
            tui.Bounds{ .x = start_x, .y = start_y, .width = 50, .height = log_height + 2 }, 
            tui.BoxStyle.rounded
        );
