//! SmartDashboard - Advanced dashboard with progressive rendering capabilities
//! 
...
/// Smart dashboard that adapts to terminal capabilities
pub const SmartDashboard = struct {
    const Self = @This();
    ...
    pub const AdaptiveLayout = struct {
        ...
        pub fn recalculate(self: *AdaptiveLayout, bounds: Bounds, config: Config) void {
            ...
            self.cell_size = Point.init(
                @divFloor(bounds.width, @as(i32, @intCast(config.grid_columns))),
                @divFloor(bounds.height, @as(i32, @intCast(config.grid_rows)))
            );
    ...
    fn renderPerformanceInfo(self: *Self, renderer: *Renderer, ctx: RenderContext) !void {
        ...
        const perf_text = try std.fmt.bufPrint(
            perf_buffer[0..], 
            "Render: {d:.1}ms | Widgets: {d} | Mode: {s}", 
            .{ self.performance_stats.average_render_time_ms, self.widgets.items.len, @tagName(self.render_mode) }
        );
