...
/// Extract content from a heading section
fn extractHeadingSection(allocator: std.mem.Allocator, content: []const u8, heading_text: []const u8) ![]const u8 {
    ...
    var section_lines = std.ArrayList([]const u8).init(allocator);
    defer section_lines.deinit();

    var current_heading_level: ?usize = null;
    ...
    while (lines.next()) |line| {
        ...
        if (trimmed.len > 0 and trimmed[0] == '#') {
            ...
            const heading_content = std.mem.trim(u8, trimmed[level..], " \t");

            if (!found_section and std.mem.indexOf(u8, heading_content, heading_text) != null) {
                found_section = true;
                current_heading_level = level;
                try section_lines.append(line);
                continue;
            }

            if (found_section and level <= current_heading_level.?) {
                // Found next section at same or higher level, stop here
                break;
            }
        }
    ...
    for (section_lines.items, 0..) |line, i| {
...
fn extractFrontMatter(content: []const u8) []const u8 {
    ...
    if (trimmed.len < 6) return ""; // Minimum: "---\n\n---"
...
/// Find files by glob pattern (simplified)
fn findFilesByPattern(allocator: std.mem.Allocator, pattern: []const u8) ![][]const u8 {
    ...
    if (std.mem.startsWith(u8, pattern, "*.")) {
        const extension = pattern[1..]; // Include the dot

        const entries = try fs.listDir(allocator, ".", 1000);
        defer {
            for (entries) |entry| allocator.free(entry);
            allocator.free(entries);
        }
