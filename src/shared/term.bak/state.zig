/// Enhanced terminal state management with cross-platform support
...
fn makeRawWindows(fd: std.posix.fd_t) !TerminalState {
    ...
    if (std.os.windows.kernel32.GetConsoleMode(handle, &inputMode) == 0) {
    ...
    if (std.os.windows.kernel32.GetConsoleMode(stdout_handle, &outputMode) == 0) {
        return error.GetConsoleModeFailed;
    }
    ...
    // Set raw input mode
    const new_inputMode = inputMode & ~@as(std.os.windows.DWORD, std.os.windows.ENABLE_ECHO_INPUT |
        std.os.windows.ENABLE_LINE_INPUT |
        std.os.windows.ENABLE_PROCESSED_INPUT);

    if (std.os.windows.kernel32.SetConsoleMode(handle, new_inputMode) == 0) {
...
fn setStateWindows(fd: std.posix.fd_t, state: *const WindowsState) !void {
    ...
    if (std.os.windows.kernel32.SetConsoleMode(handle, state.inputMode) == 0) {
        return error.SetConsoleModeFailed;
    }

    const stdout_handle = std.os.windows.kernel32.GetStdHandle(std.os.windows.STD_OUTPUT_HANDLE);
    if (std.os.windows.kernel32.SetConsoleMode(stdout_handle, state.outputMode) == 0) {
        return error.SetConsoleModeFailed;
    }
}
