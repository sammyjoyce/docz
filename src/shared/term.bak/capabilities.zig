...
const FieldMapping = struct {
    ...
    fn mapField(comptime field_name: []const u8) []const u8 {
        ...
        if (std.mem.eql(u8, field_name, "supportsTermOsc133")) return "supports_term_osc133";
...
fn defaultsCaps() TermCaps {
    ...
    return TermCaps{
        ...
        .supportsTermOsc133 = d.supports_term_osc133,
...
fn applyOverlay(prog_config: anytype, caps: *TermCaps) void {
    ...
    inline for (std.meta.fields(TermCaps)) |field| {
        const source_field_name = FieldMapping.mapField(field.name);
...
test "field mapping works correctly" {
    ...
    try testing.expectEqualStrings("supports_truecolor", FieldMapping.mapField("supportsTruecolor"));
    try testing.expectEqualStrings("supports_kitty_keyboard", FieldMapping.mapField("supportsKittyKeyboard"));
    try testing.expectEqualStrings("width_method", FieldMapping.mapField("widthMethod"));
    try testing.expectEqualStrings("screen_chunk_limit", FieldMapping.mapField("screenChunkLimit"));
}
