//! Terminal Interface
//!
//! This module provides a high-level interface to all terminal capabilities,
...
const ansi_color = @import("ansi/color/mod.zig");
...
pub const Terminal = struct {
    ...
    pub fn showCursor(self: *Self, visible: bool) !void {
        if (visible) {
            try ansi_mode.showCursor(self.writer, self.caps);
        } else {
            try ansi_mode.hideCursor(self.writer, self.caps);
        }
    }
    ...
    fn renderImageKitty(self: *Self, image: Image, width: i32, height: i32) !void {
        // Use the centralized Kitty Graphics Protocol implementation
        ...
        // Build options for ansi_graphics
        const opts = [_][]const u8{
            format_code,
            std.fmt.allocPrint(self.allocator, "s={d}", .{width}) catch "s=100",
            std.fmt.allocPrint(self.allocator, "v={d}", .{height}) catch "v=100",
        };
        defer {
            for (opts[1..]) |opt| {
        ...
        try ansi_graphics.writeKittyGraphics(&self.writer, self.allocator, self.caps, &opts, encoded);
    }
    ...
    fn encodeSixelRgb24(self: *Self, allocator: std.mem.Allocator, image: Image, target_width: u32, target_height: u32) !void {
        ...
        // Initialize color palette (simplified)
        const colors = [_][3]u8{
            .{ 0, 0, 0 }, // Black
            .{ 255, 0, 0 }, // Red
            .{ 0, 255, 0 }, // Green
...
pub const DashboardMode = struct {
    ...
    pub fn detect(caps: TermCaps) DashboardMode {
        return DashboardMode{
            ...
            .interactions = if (caps.supportsSgrPixelMouse) .full_mouse else if (caps.supportsSgrMouse) .mouse else .keyboard_only,
