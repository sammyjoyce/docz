...
pub const KeyEvent = struct {
    ...
    pub fn format(self: KeyEvent, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        ...
        const keyCode = self.base_code orelse self.code;
        try writer.print("{s}", .{keyCode.getName()});
    }
};
