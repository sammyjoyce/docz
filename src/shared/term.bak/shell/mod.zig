...
/// Provides an interface for shell integration across different terminal emulators
///
/// This module consolidates shell integration functionality from various sources:
/// - Term protocol (OSC 133)
...
pub const TermInterface = term.TermInterface;
...
/// Create a shell integration with auto-detected capabilities
pub fn create(allocator: std.mem.Allocator) ShellIntegration {
...
test "shell integration module" {
    ...
    try std.testing.expect(caps.supports_term);

    // Test creation
    var manager = create(allocator);
