...
// Input handling system with modern terminal features
...
pub fn textKey(text: []const u8) Key {
    const code = if (text.len == 1) @as(u21, text[0]) else KEY_EXTENDED;
...
pub const InputParser = struct {
    ...
    fn parseAltSequence(self: *InputParser) InputEvent {
        if (self.buffer.items.len >= 2) {
            ...
            var key_event = fromCode(@as(u21, char_byte));
...
test "key events" {
    ...
    const test_key = fromCode('x');
    const press_event = keyPress(test_key);
    const release_event = keyRelease(test_key);
