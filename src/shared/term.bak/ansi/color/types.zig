...
pub const HexColor = struct {
    ...
    pub fn init(value: u32) HexColor {
        return .{ .value = value };
    }

    pub fn fromString(str: []const u8) !HexColor {
        var hex_str = str;
        ...
        if (hex_str.len == 3) {
            ...
            return init((@as(u32, r) << 16) | (@as(u32, g) << 8) | @as(u32, b));
    ...
    pub fn toRgb(self: HexColor) RGB {
        return RGB.init(
            @intCast((self.value >> 16) & 0xFF),
            @intCast((self.value >> 8) & 0xFF),
            @intCast(self.value & 0xFF),
        );
    }
