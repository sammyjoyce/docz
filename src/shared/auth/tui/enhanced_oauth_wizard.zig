//! Enhanced OAuth Wizard with Rich TUI Experience
//!
...
const smart_progress = @import("tui_shared").widgets.Enhanced.ProgressBar;
const smart_notification = @import("tui_shared").widgets.Enhanced.Notification;
const enhanced_text_input = @import("tui_shared").widgets.Enhanced.TextInput;
const status_bar = @import("tui_shared").dashboard.StatusBar;
const renderer_mod = @import("tui_shared").renderer;
const bounds_mod = @import("tui_shared").bounds;
const input_system = @import("tui_shared").input;
const term_mod = @import("term_shared").unified;

// Re-export types for convenience
const SmartProgressBar = smart_progress.SmartProgressBar;
const SmartNotification = smart_notification.SmartNotification;
const SmartNotificationController = smart_notification.SmartNotificationController;
const EnhancedTextInput = enhanced_text_input.EnhancedTextInput;
...
pub const EnhancedOAuthWizard = struct {
    ...
    renderer: *Renderer,
    notification_controller: SmartNotificationController,
    progress_bar: SmartProgressBar,
    text_input: ?EnhancedTextInput = null,
    ...
    pub fn init(allocator: std.mem.Allocator, renderer: *Renderer) !Self {
        ...
        const notification_controller = SmartNotificationController.init(allocator, renderer);

        // Initialize progress bar
        const progress_bar = SmartProgressBar.init("OAuth Setup", .gradient);
...
pub fn runEnhancedOAuthWizard(allocator: std.mem.Allocator) !oauth.OAuthCredentials {
    // Create renderer
    const renderer = try renderer_mod.createRenderer(allocator);
    defer renderer.deinit();

    // Create and run wizard
    var wizard = try EnhancedOAuthWizard.init(allocator, renderer);
    defer wizard.deinit();
...
pub fn setupOAuthWithEnhancedTUI(allocator: std.mem.Allocator) !oauth.OAuthCredentials {
    return try runEnhancedOAuthWizard(allocator);
}
