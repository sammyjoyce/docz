...
pub fn parseAndHandle(allocator: Allocator, args: [][]const u8) !?ParsedArgs {
    ...
    // Handle ONLY help and version commands - let auth commands bubble up to agent
    if (parsed.command) |cmd| {
        switch (cmd) {
            .help => {
            ...
            .auth => {
                // Auth commands should bubble up to the agent for proper OAuth flow
                return parsed;
            },
            .chat, .interactive, .tui_demo => {
                // These commands should be handled by the caller
                return parsed;
            },
        }
    } else if (parsed.prompt == null) {
    ...
    // Return parsed args if there's a prompt to process or an auth command
    return parsed;
}
