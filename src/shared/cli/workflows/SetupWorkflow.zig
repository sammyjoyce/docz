...
pub const Config = struct {
    configDir: []const u8 = "~/.config/docz",
    createConfigDir: bool = true,
    setupAuth: bool = true,
    setupThemes: bool = true,
...
pub const SetupWorkflow = struct {
    ...
    pub fn init(allocator: Allocator, config: Config) SetupWorkflow {
        return .{
            .allocator = allocator,
            .config = config,
            .runner = Runner.Runner.init(allocator, undefined),
        };
    }
    ...
    pub fn execute(self: *SetupWorkflow) !Runner.Result {
        var steps = std.ArrayList(Step.Step).init(self.allocator);
        defer steps.deinit();
        ...
        if (self.config.createConfigDir) {
            try steps.append(Step.Step{
        ...
        // Check dependencies
        if (self.config.checkDependencies) {
            try steps.append(Step.Step{
        ...
        return try self.runner.execute("Initial Setup");
    }

    /// Step 1: Check system requirements
    fn checkSystemRequirements(allocator: std.mem.Allocator, context: ?Step.Context) !Step.StepResult {
        _ = context;

        // Check OS version and system info
        const osInfo = try std.fs.selfExePathAlloc(allocator);
        defer allocator.free(osInfo);

        // Check available disk space (simplified)
        const cwd = std.fs.cwd();
        const stat = try cwd.statFile(".");
        const diskSpaceKb = stat.size / 1024;
        ...
        const outputData = try std.fmt.allocPrint(allocator, "OS: {s}, Terminal: {s}, Disk: {d}KB", .{
            std.Target.current.os.tag,
            term,
            diskSpaceKb,
        });
    ...
    fn createConfigDirectory(allocator: std.mem.Allocator, context: ?Step.Step) !Step.StepResult {
        const configDir = if (context) |ctx| ctx.getParam("config_dir") orelse "~/.config/docz" else "~/.config/docz";

        // Expand ~ to home directory
        const homeDir = std.process.getEnvVarOwned(allocator, "HOME") catch |err| switch (err) {
            error.EnvironmentVariableNotFound => {
        ...
        const expandedDir = try std.fs.path.join(allocator, &.{ homeDir, std.fs.path.basename(configDir) });
        defer allocator.free(expandedDir);

        // Create the directory
        std.fs.cwd().makeDir(expandedDir) catch |err| switch (err) {
            error.PathAlreadyExists => {},
            else => return err,
        };

        const outputData = try std.fmt.allocPrint(allocator, "Created config directory: {s}", .{expandedDir});
    ...
    fn checkDependencies(allocator: std.mem.Allocator, context: ?Step.Step) !Step.StepResult {
        _ = context;
    ...
    fn initializeConfig(allocator: std.mem.Allocator, context: ?Step.Step) !Step.StepResult {
        _ = context;
        ...
        const outputData = try allocator.dupe(u8, "Configuration files initialized");
    ...
    fn setupThemes(allocator: std.mem.Allocator, context: ?Step.Step) !Step.StepResult {
        _ = context;
    ...
    fn setupAuthentication(allocator: std.mem.Allocator, context: ?Step.Step) !Step.StepResult {
        _ = context;
        ...
        const outputData = try allocator.dupe(u8, "Authentication setup skipped (would prompt interactively)");
    ...
    fn verifyInstallation(allocator: std.mem.Allocator, context: ?Step.Step) !Step.StepResult {
        _ = context;
        ...
        const outputData = try std.fmt.allocPrint(allocator, "Verification: {d}/{d} checks passed", .{ checks_passed, total_checks });
