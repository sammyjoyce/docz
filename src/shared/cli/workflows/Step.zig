...
pub const Context = struct {
    ...
    pub fn getParam(self: Context, key: []const u8) ?[]const u8 {
...
pub const Step = struct {
    ...
    executeFn: *const fn (allocator: Allocator, context: ?Context) anyerror!StepResult,
    ...
    pub fn withContext(self: Step, ctx: Context) Step {
        return .{
            .name = self.name,
    ...
    pub fn withTimeout(self: Step, timeoutMs: u32) Step {
        return .{
            .name = self.name,
            .description = self.description,
            .executeFn = self.executeFn,
    ...
    pub fn withRetry(self: Step, retryCount: u32) Step {
        return .{
            .name = self.name,
            .description = self.description,
            .executeFn = self.executeFn,
...
/// Steps for typical CLI operations
pub const Steps = struct {
    /// Delay step for testing or spacing
    pub fn delay(durationMs: u32) Step {
        const DelayImpl = struct {
            fn execute(allocator: Allocator, context: ?Context) anyerror!StepResult {
                _ = allocator;
                _ = context;
    ...
    pub fn checkFileExists(filePath: []const u8) Step {
    ...
    pub fn createDirectory(dirPath: []const u8) Step {
    ...
    pub fn checkEnvironmentVariable(varName: []const u8) Step {
        const CheckEnvImpl = struct {
            fn execute(allocator: Allocator, context: ?Context) anyerror!StepResult {
                const name = if (context) |ctx| ctx.getParam("var_name") orelse varName else varName;

                const envValue = std.process.getEnvVarOwned(allocator, name) catch |err| switch (err) {
    ...
    pub fn validateConfiguration(configPath: []const u8) Step {
        const ValidateConfigImpl = struct {
            fn execute(allocator: Allocator, context: ?Context) anyerror!StepResult {
                _ = context;

                // Read and validate configuration file
                const file = std.fs.cwd().openFile(configPath, .{}) catch |err| switch (err) {
                    error.FileNotFound => return StepResult{
                        .success = false,
                ...
                const contents = try file.readToEndAlloc(allocator, fileSize);
...
/// Step builder for creating steps with fluent API
pub const Builder = struct {
    ...
    pub fn description(self: Builder, desc: []const u8) Builder {
        return .{
    ...
    pub fn timeout(self: Builder, timeoutMs: u32) Builder {
        return .{
    ...
    pub fn retry(self: Builder, retryCount: u32) Builder {
        return .{
    ...
    pub fn execute(self: Builder, executeFn: *const fn (allocator: Allocator, context: ?Context) anyerror!StepResult) Step {
        return .{
            .name = self.step.name,
