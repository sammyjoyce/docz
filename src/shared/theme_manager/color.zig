...
pub const Color = struct {
    ...
    pub fn init(name: []const u8, rgb: ansi_color.RgbColor, alpha: f32) Self {
        const ansi256 = ansi_color.rgbToAnsi256(rgb);
        const ansi16 = ansi_color.ansi256ToAnsi16(ansi256);
    ...
    pub fn fromHex(name: []const u8, hex: []const u8, alpha: f32) !Self {
        const hex_color = try ansi_color.HexColor.fromString(hex);
    ...
    pub fn fromHsl(name: []const u8, h: f32, s: f32, l: f32, alpha: f32) Self {
        const hsl = ansi_color.HslColor.init(h, s, l);
        const rgb = hsl.toRgb();
        return init(name, rgb, alpha);
    }
    ...
    pub fn fromHsv(name: []const u8, h: f32, s: f32, v: f32, alpha: f32) Self {
        const hsv = ansi_color.HsvColor.init(h, s, v);
        const rgb = hsv.toRgb();
        return init(name, rgb, alpha);
    }
