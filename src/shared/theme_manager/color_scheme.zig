...
pub const ColorScheme = struct {
    ...
    pub fn fromZon(allocator: std.mem.Allocator, content: []const u8) !*Self {
        ...
        const primary = try zonColorToColor(allocator, parsed.colors.primary);
        const secondary = try zonColorToColor(allocator, parsed.colors.secondary);
...
test "ZON parsing" {
    ...
    const zonContent =
        ...
        \\        .primary = .{ .name = "primary", .r = 0, .g = 123, .b = 255, .alpha = 1.0 },
        \\        .secondary = .{ .name = "secondary", .r = 108, .g = 117, .b = 125, .alpha = 1.0 },
    ...
    try testing.expectEqual(@as(u8, 0), theme.primary.rgb.r);
    try testing.expectEqual(@as(u8, 123), theme.primary.rgb.g);
    try testing.expectEqual(@as(u8, 255), theme.primary.rgb.b);
}
