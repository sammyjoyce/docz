...
const enhanced_input = @import("../../term/enhanced_input_handler.zig");
    ...
    fn renderCompositeUnicode(self: *Self) !void {
        ...
        for (0..self.viewport.bounds.height) |y| {
            for (0..self.viewport.bounds.width) |x| {
                const world_point = self.viewport.screenToWorld(.{ .x = @as(f32, @floatFromInt(x)), .y = @as(f32, @floatFromInt(y)) });
