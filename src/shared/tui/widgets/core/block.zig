...
pub const Block = struct {
    ...
    fn drawBorder(self: *const Block) void {
        ...
        // Set border color
        print("\x1b[{d}m", .{@intFromEnum(self.border_color)});

        // Top border with title
        moveCursor(self.bounds.y, self.bounds.x);
        print("{s}", .{chars.top_left});
