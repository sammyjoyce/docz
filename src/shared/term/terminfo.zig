...
const TerminfoParser = struct {
    ...
    fn mapStringCapabilities(_: *TerminfoParser, db: *Database, offsets: []const u8, table: []const u8, count: u16) !void {
        ...
        const capabilities = [_]struct { name: []const u8, index: u16 }{
            ...
            .{ .name = "kcud1", .index = cap_indices.kcud1 },
            .{ .name = "kcub1", .index = cap_indices.kcub1 },
            .{ .name = "kcuf1", .index = cap_indices.kcuf1 },
            .{ .name = "khome", .index = cap_indices.khome },
            .{ .name = "kend", .index = cap_indices.kend },
            .{ .name = "kpp", .index = cap_indices.kpp },
            .{ .name = "knp", .index = cap_indices.knp },
            .{ .name = "kich1", .index = cap_indices.kich1 },
            .{ .name = "kdch1", .index = cap_indices.kdch1 },
            .{ .name = "kf1", .index = cap_indices.kf1 },
            .{ .name = "kf2", .index = cap_indices.kf2 },
            .{ .name = "kf3", .index = cap_indices.kf3 },
            .{ .name = "kf4", .index = cap_indices.kf4 },
            .{ .name = "kf5", .index = cap_indices.kf5 },
            .{ .name = "kf6", .index = cap_indices.kf6 },
            .{ .name = "kf7", .index = cap_indices.kf7 },
            .{ .name = "kf8", .index = cap_indices.kf8 },
            .{ .name = "kf9", .index = cap_indices.kf9 },
            .{ .name = "kf10", .index = cap_indices.kf10 },
    ...
    fn mapBooleanCapabilities(_: *TerminfoParser, db: *Database, bools: []const u8, _: u16) !void {
        ...
        if (bool_indices.auto_right_margin < bools.len and bools[bool_indices.auto_right_margin] != 0) {
            try db.addCapability("am", "true");
        }
        if (bool_indices.can_change < bools.len and bools[bool_indices.can_change] != 0) {
