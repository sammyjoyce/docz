...
/// Graphics renderer that handles image rendering and caching
pub const Graphics = struct {
    ...
    fn renderKittyImage(self: *Self, image: CachedImage, width: u32, height: u32, options: RenderOptions) !void {
        ...
        // Build options array for ansi_graphics
        const opts = [_][]const u8{
            std.fmt.allocPrint(self.allocator, "{s}={s}", .{ transmission_medium, transmission_medium }) catch "t=t",
            format_code,
        ...
        defer {
            for (opts) |opt| {
                if (std.mem.indexOf(u8, opt, "=") != null) {
        ...
        try ansi_graphics.writeKittyGraphics(self.terminal.writer, self.allocator, caps, &opts, encoded);
    }
    ...
    fn renderSixelImage(self: *Self, image: CachedImage, width: u32, height: u32, options: RenderOptions) !void {
        ...
        // properly encode the image data to Sixel format using ansi/sixel_graphics.zig
        const sixel_data = try self.convertToSixel(image, width, height);
        defer self.allocator.free(sixel_data);

        const caps = self.terminal.getCapabilities();
        try ansi_graphics.writeSixelGraphics(self.terminal.writer, self.allocator, caps, 0, 0, 0, sixel_data);

        _ = options; // Options could be used for background color, transparency, etc.
    }
