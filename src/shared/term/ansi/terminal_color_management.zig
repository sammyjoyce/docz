...
pub const TerminalColor = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, caps: caps_mod.TermCaps) !*TerminalColor {
        const self = try allocator.create(TerminalColor);
        self.* = TerminalColor{
            .allocator = allocator,
            .caps = caps,
    ...
    pub fn deinit(self: *TerminalColor) void {
    ...
    fn initPaletteCache(self: *TerminalColor) !void {
    ...
    pub fn getColorCapability(self: *TerminalColor) ColorCapability {
    ...
    pub fn setForegroundColor(self: *TerminalColor, r: u8, g: u8, b: u8) ![]u8 {
    ...
    pub fn setBackgroundColor(self: *TerminalColor, r: u8, g: u8, b: u8) ![]u8 {
        const rgb_key = (@as(u32, r) << 16) | (@as(u32, g) << 8) | @as(u32, b);
    ...
    pub fn createGradient(self: *TerminalColor, start_r: u8, start_g: u8, start_b: u8, end_r: u8, end_g: u8, end_b: u8, steps: u8) ![]u8 {
    ...
    /// Apply color scheme
    pub fn applyColorScheme(self: *TerminalColor, scheme: ColorScheme) ![]u8 {
        var result = std.ArrayList(u8).init(self.allocator);
        errdefer result.deinit();
    ...
    pub fn queryColors(_: *TerminalColor) !struct { fg: ?precise_palette.RGBColor, bg: ?precise_palette.RGBColor } {
    ...
    fn formatBasicBg(self: *TerminalColor, index: u8) ![]u8 {
        if (index <= 7) {
    ...
    /// Set cursor color
    fn setCursorColor(self: *TerminalColor, r: u8, g: u8, b: u8) ![]u8 {
        // OSC 12 for cursor color
        return try std.fmt.allocPrint(self.allocator, "\x1b]12;#{:0>6x}\x07", .{(@as(u32, r) << 16) | (@as(u32, g) << 8) | @as(u32, b)});
    }
};
