...
//! Provides comprehensive cursor positioning, styling, and state management
...
// Visibility and Blinking functions are now in ansi/mode.zig
...
/// RGB color representation for cursor color (re-exported for convenience)
...
pub const CursorController = struct {
    ...
    pub fn setVisible(self: *CursorController, visible: bool) !void {
        ...
        if (visible) {
            try @import("mode.zig").showCursor(self.writer, self.caps);
        } else {
            try @import("mode.zig").hideCursor(self.writer, self.caps);
        }
    }
...
test "cursor position string generation" {
    ...
    // Test home position
    const home_seq = try cursorPositionString(allocator, 0, 0);
    defer allocator.free(home_seq);
    try std.testing.expectEqualStrings("\x1b[H", home_seq);

    // Test specific position
    const pos_seq = try cursorPositionString(allocator, 10, 20);
    defer allocator.free(pos_seq);
...
test "cursor style string generation" {
    ...
    const style_seq = try setCursorStyleString(allocator, .steady_block);
    defer allocator.free(style_seq);
    try std.testing.expectEqualStrings("\x1b[2 q", style_seq);
}
