//! Unified Cursor System - High-Level API
//!
//! This module provides a unified interface for cursor control, combining
...
//! - High-level: THIS FILE (unified API that uses both)
...
// ============================================================================
// UNIFIED CURSOR CONTROLLER (Combines Input and Output)
// ============================================================================

/// Unified cursor controller that handles both input and output
pub fn UnifiedCursorControllerFor(comptime Writer: type) type {
    return struct {
        ...
        /// Initialize unified cursor controller
...
pub const UnifiedCursorController = UnifiedCursorControllerFor(std.fs.File.Writer);

// ============================================================================
// CONVENIENCE FUNCTIONS
// ============================================================================

/// Simple function to hide cursor (no capabilities required)
pub fn hide(writer: anytype) !void {
    try writer.writeAll(HIDE_CURSOR);
}

/// Simple function to show cursor (no capabilities required)
...
/// Simple function to move cursor home
pub fn home(writer: anytype) !void {
    try writer.writeAll(control_cursor.CURSOR_HOME_POSITION);
}

/// Simple function to request cursor position
...
test "unified controller initialization" {
    ...
    const caps = TermCaps{};
    var controller = UnifiedCursorController.init(allocator, test_writer, caps);
