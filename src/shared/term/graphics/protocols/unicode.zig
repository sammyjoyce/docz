        ...
        while (y < char_height) : (y += 1) {
            ...
            while (x < char_width) : (x += 1) {
                ...
                if (unicode_opts.use_color and caps.supportsColor) {
                    ...
                    if (caps.supportsRgb) {
                        try std.fmt.format(line.writer(), "\x1b[38;2;{d};{d};{d}m\x1b[48;2;{d};{d};{d}m", .{ fg_color.r, fg_color.g, fg_color.b, bg_color.r, bg_color.g, bg_color.b });
                    }
                }
    ...
    pub fn render(
        ...
        while (y < target_height) : (y += 1) {
            ...
            while (x < target_width) : (x += 1) {
                ...
                const char_idx = @min(ASCII_CHARS.len - 1, @as(usize, @intFromFloat(brightness * @as(f32, @floatFromInt(ASCII_CHARS.len)))));
