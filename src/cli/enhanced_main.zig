...
const enhanced = @import("components/enhanced/mod.zig");
const demo = @import("demos/enhanced_capabilities_demo.zig");
...
pub const EnhancedCliApp = struct {
    ...
    notifications: enhanced.EnhancedNotification,
    ...
    pub fn init(allocator: std.mem.Allocator) !Self {
        ...
        const notification_config = enhanced.NotificationConfig{
        ...
        const notifications = enhanced.EnhancedNotification.init(&bridge, notification_config);
    ...
    fn runNotifyCommand(self: *Self, args: []const []const u8) !void {
        ...
        const notification_type = if (std.mem.eql(u8, type_str, "info"))
            enhanced.NotificationType.info
        else if (std.mem.eql(u8, type_str, "success"))
            enhanced.NotificationType.success
        else if (std.mem.eql(u8, type_str, "warning"))
            enhanced.NotificationType.warning
        else if (std.mem.eql(u8, type_str, "error"))
            enhanced.NotificationType.@"error"
        else if (std.mem.eql(u8, type_str, "critical"))
            enhanced.NotificationType.critical
        else
            enhanced.NotificationType.info;
    ...
    fn runInputCommand(self: *Self, args: []const []const u8) !void {
        ...
        if (std.mem.eql(u8, input_type, "email")) {
            var input = enhanced.SmartInputPresets.email(&self.bridge);
            defer input.deinit();
    ...
    fn handleError(self: *Self, command: []const u8, err: anyerror) !void {
        ...
        const actions = [_]enhanced.NotificationAction{
            .{
                .label = "View Help",
        ...
        try self.notifications.showWithActions(
            .@"error",
            "Command Error",
            error_msg,
            &actions
