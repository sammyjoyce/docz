...
pub const Workflow = struct {
    ...
    pub fn init(name: []const u8, description: []const u8, steps: []const WorkflowStep.Step) Workflow {
        return Workflow{
            .name = name,
...
pub const WorkflowRegistry = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, ctx: *context.CliContext) WorkflowRegistry {
    ...
    pub fn execute(self: *WorkflowRegistry, workflow_name: []const u8) !types.CommandResult {
        ...
        for (workflow.steps, 0..) |step, i| {
            ...
            if (self.context.verbose) {
                self.context.verbose_log("Executing step {d}/{d}: {s}", .{ step_num, workflow.steps.len, step.name });
            }
    ...
    pub fn registerCommonWorkflows(self: *WorkflowRegistry) !void {
        // Auth workflow
        const auth_steps = [_]WorkflowStep.Step{
        ...
        try self.register(Workflow.init(
            "auth-setup",
            "Set up authentication with API service",
            &auth_steps
        ).withCategory(.auth));
        
        // Configuration workflow
        const config_steps = [_]WorkflowStep.Step{
        ...
        // Setup workflow
        const setup_steps = [_]WorkflowStep.Step{
...
fn createAuthVerificationStep() WorkflowStep.Step {
    const AuthVerifyImpl = struct {
        fn execute(allocator: std.mem.Allocator, ctx: ?WorkflowStep.StepContext) anyerror!WorkflowStep.StepResult {
            _ = allocator;
            _ = ctx;
    ...
    return WorkflowStep.Step.init("Verify Authentication", AuthVerifyImpl.execute)
        .withDescription("Verify API key and authentication status");
}

fn createConfigOptimizationStep() WorkflowStep.Step {
    ...
    return WorkflowStep.Step.init("Optimize Configuration", ConfigOptimizeImpl.execute)
        .withDescription("Optimize configuration for current environment");
}

fn createInitialConfigStep() WorkflowStep.Step {
    ...
    return WorkflowStep.Step.init("Create Initial Configuration", InitConfigImpl.execute)
