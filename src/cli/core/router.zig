...
pub const CommandRouter = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, ctx: *context.CliContext) !CommandRouter {
        // Note: ctx parameter is const in init but we need to store a mutable reference
        // This is safe because we're storing the pointer, not modifying the pointed-to value
        const ctx_ptr = @as(*context.CliContext, @constCast(ctx));
        
        var workflow_registry = workflows.WorkflowRegistry.init(allocator, ctx_ptr);
        ...
        if (args.raw_message) |msg| {
            ...
            while (stage_iter.next()) |stage| {
                ...
                if (self.context.verbose) {
                    self.context.verbose_log("Pipeline stage: {s}", .{trimmed_stage});
                }
    ...
    fn executeVersion(self: *CommandRouter, args: types.ParsedArgsUnified) !types.CommandResult {
        ...
        const version_info = try std.fmt.allocPrint(
            self.allocator,
            "docz 1.0.0 - Enhanced CLI\nTerminal: {s}\nFeatures: {}",
            .{ self.context.capabilitySummary(), self.context.capabilities }
        );
