...
/// Enhanced 256-color conversion algorithm inspired by charmbracelet/x
/// Uses advanced color science for optimal terminal color approximation
const RGBColor = enhanced_distance.RGBColor;

/// Advanced 256-color converter using charmbracelet's algorithm
pub const CharmbraceLetColorConverter = struct {
    const Self = @This();

    /// 6x6x6 color cube values used by xterm (exact values from charmbracelet)
    ...
    fn calculateGrayscaleColor(rgb: RGBColor) struct { color: RGBColor, index: u8 } {
        // Calculate average (simple approach) - charmbracelet uses more sophisticated
    ...
    /// This closely follows the charmbracelet/x Convert256 function
    pub fn convert256Enhanced(rgb: RGBColor) u8 {
        ...
        // Check if we have an exact cube match (optimization from charmbracelet)
    ...
    /// Convert to 16-color using the precise mapping from charmbracelet
...
pub const ColorConverter = struct {
    ...
    pub fn convert(rgb: RGBColor, strategy: Strategy) u8 {
        return switch (strategy) {
            .enhanced => CharmbraceLetColorConverter.convert256Enhanced(rgb),
            .fast => CharmbraceLetColorConverter.convert256Fast(rgb),
            .optimal => CharmbraceLetColorConverter.convertOptimal(rgb),
    ...
    pub fn convert16(rgb: RGBColor) u8 {
        return CharmbraceLetColorConverter.convert16(rgb);
    }
...
test "cube coordinate calculation" {
    ...
    try testing.expect(CharmbraceLetColorConverter.rgbTo6Cube(0) == 0);
    try testing.expect(CharmbraceLetColorConverter.rgbTo6Cube(47) == 0); // Below threshold
    try testing.expect(CharmbraceLetColorConverter.rgbTo6Cube(48) == 1); // At threshold
    try testing.expect(CharmbraceLetColorConverter.rgbTo6Cube(255) == 5); // Max value

    // Test cube-to-RGB conversion
    try testing.expect(CharmbraceLetColorConverter.cubeToRgb(0) == 0x00);
    try testing.expect(CharmbraceLetColorConverter.cubeToRgb(1) == 0x5F);
    try testing.expect(CharmbraceLetColorConverter.cubeToRgb(5) == 0xFF);
}

test "cube color calculation" {
    const pure_red = RGBColor.init(255, 0, 0);
    const cube_result = CharmbraceLetColorConverter.calculateCubeColor(pure_red);
...
test "grayscale calculation" {
    ...
    const gray_result = CharmbraceLetColorConverter.calculateGrayscaleColor(gray);
...
test "enhanced color conversion" {
    ...
    const pure_red = RGBColor.init(255, 0, 0);
    const red_result = CharmbraceLetColorConverter.convert256Enhanced(pure_red);
    try testing.expect(red_result == 9 or red_result == 196); // Basic red or cube red

    const pure_blue = RGBColor.init(0, 0, 255);
    const blue_result = CharmbraceLetColorConverter.convert256Enhanced(pure_blue);
    ...
    const gray_result = CharmbraceLetColorConverter.convert256Enhanced(mid_gray);
...
test "conversion analysis" {
    ...
    const analysis = CharmbraceLetColorConverter.Analysis.analyzeConversion(test_color);
...
test "batch conversion" {
    ...
    CharmbraceLetColorConverter.convertBatch(&test_colors, &results, false);
