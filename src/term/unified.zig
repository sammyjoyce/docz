...
pub const Terminal = struct {
    ...
    fn renderImageKitty(self: *Self, image: Image, width: i32, height: i32) !void {
        ...
        try self.writer.print(
            "\x1b_G{s},s={d},v={d};{s}\x1b\\",
            .{ format_code, width, height, encoded }
        );
    }
    
    fn renderImageSixel(self: *Self, image: Image, width: i32, height: i32) !void {
        // Basic Sixel rendering (simplified implementation)
        _ = image;
        _ = width;
        _ = height;
        
        // For now, just indicate Sixel support
        try self.writer.writeAll("\x1bP0;0;0q");
        // TODO: Implement full Sixel encoding
    ...
    pub fn flush(self: *Self) !void {
        _ = self; // No explicit flush needed for stdout, but this can be extended
    }
...
pub const DashboardMode = struct {
    ...
    pub fn detect(caps: TermCaps) DashboardMode {
        return DashboardMode{
            .graphics = if (caps.supportsKittyGraphics) .kitty_graphics
                       else if (caps.supportsSixel) .sixel_graphics  
                       else if (caps.supportsUnicode) .unicode_enhanced
                       else .ascii_fallback,
            .colors = if (caps.supportsTrueColor) .truecolor
                     else if (caps.supports256Color) .palette_256
                     else .ansi_16,
            .interactions = if (caps.supportsSgrPixelMouse) .full_mouse
                          else if (caps.supportsSgrMouse) .basic_mouse
                          else .keyboard_only,
...
pub const DashboardTerminal = struct {
    ...
    terminal: Terminal,
    mode: DashboardMode,
    performance_buffer: std.ArrayList(u8),
    ...
    pub fn init(allocator: std.mem.Allocator) !Self {
        ...
        return Self{
            .terminal = terminal,
            .mode = mode,
            .performance_buffer = std.ArrayList(u8).init(allocator),
        };
    }
    
    pub fn deinit(self: *Self) void {
        self.performance_buffer.deinit();
    ...
    pub fn copyTableData(self: *Self, data: []const []const []const u8, format: ClipboardFormat) !void {
        ...
        const writer = self.performance_buffer.writer();
