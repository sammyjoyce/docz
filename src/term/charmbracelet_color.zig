...
// Sophisticated color handling system inspired by charmbracelet/x
...
fn findNearest256(rgb_color: RGBColor) IndexedColor {
    ...
    const cube_dist = weightedColorDistance(Color{ .rgb = rgb_color }, Color{ .rgb = cube_color });
    const grey_dist = weightedColorDistance(Color{ .rgb = rgb_color }, Color{ .rgb = grey_color });
...
const ansi256To16 = [_]BasicColor{
    ...
    .bright_white,
    // Continue mapping for all 256 colors...
    // This is a simplified version - full implementation would map all 256 colors
} ++ [_]BasicColor{.black} ** (256 - 16); // Fill remaining with black for now
