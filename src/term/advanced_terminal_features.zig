...
pub const cursor_optimizer = @import("advanced_cursor_optimizer.zig");
pub const input_handler = @import("enhanced_input_handler.zig");
...
pub const AdvancedTerminal = struct {
    ...
    buffer: CellBuffer,
    cursor_optimizer: CursorOptimizer,
    input_parser: EnhancedInputParser,
    width: usize,
    height: usize,
    cursor_x: usize = 0,
    cursor_y: usize = 0,
    ...
    pub fn enableInputLogging(self: *Self, log_fn: *const fn (ctx: ?*anyopaque, comptime format: []const u8, args: anytype) void, context: ?*anyopaque) void {
        ...
        self.input_parser.setLogger(logger);
    }
};
