...
pub const Config = struct {
    ...
    enableClipboardActions: bool = true,
...
pub const Notification = struct {
    ...
    pub fn initProgress(
        ...
        return Self{
            ...
            .persistent = true, // Progress notifications are typically persistent
        };
    }
    ...
    pub fn shouldAutoDismiss(self: *const Self) bool {
        ...
        if (self.config.autoDismissMs == null) return false;

        const now = std.time.timestamp();
        const age_ms = @as(u32, @intCast(now - self.timestamp)) * 1000;
        return age_ms > self.config.autoDismissMs.?;
    }
    ...
    pub fn getProgressPercentage(self: *const Self, allocator: std.mem.Allocator) ![]u8 {
        if (self.progress == null) return allocator.dupe(u8, "0%");
        const percentage = @as(u32, @intFromFloat(self.progress.? * 100));
        return std.fmt.allocPrint(allocator, "{d}%", .{percentage});
    }
...
pub const Util = struct {
    ...
    pub fn calculateDimensions(
        ...
        const content_width = @min(config.maxWidth - 4, @max(title_len, message_len) + 4);
        const total_width = content_width + 4; // Account for borders

        var height: u32 = 3; // Top border, title, bottom border
        height += if (config.showTimestamp) 1 else 0;
    ...
    pub fn formatProgressBar(
        allocator: std.mem.Allocator,
        progress: f32,
        width: u32,
        use_unicode: bool,
    ) ![]u8 {
        const filled_width = @as(u32, @intFromFloat(progress * @as(f32, @floatFromInt(width))));
        ...
        // Filled portion
        const filled_char = if (use_unicode) "█" else "#";
        for (0..filled_width) |_| {
            try result.appendSlice(filled_char);
        }

        // Empty portion
        const empty_char = if (use_unicode) "░" else "-";
        for (0..empty_width) |_| {
