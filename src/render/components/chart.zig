...
pub fn renderChart(renderer: *AdaptiveRenderer, chart: Chart) !void {
    ...
    const key = cacheKey("chart_{d}_{s}_{?s}", .{ 
        chart.data_series.len, @tagName(chart.chart_type), chart.title 
    });
...
fn renderUnicodeLineChart(
    renderer: *AdaptiveRenderer, 
    chart: Chart, 
    config: ChartConfig, 
    dimensions: struct { width: u16, height: u16 },
    min_max: struct { min: f64, max: f64 },
    writer: anytype
) !void {
