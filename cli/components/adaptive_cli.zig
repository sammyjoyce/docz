        ...
        while (iteration < 30) { // Limit for demo
            ...
            const stats = [_]Progress{
                .{
                    ...
                    .label = "CPU Usage",
                    .show_percentage = true,
                    .color = if (cpu_usage > 0.8) Color.ansi(.red) 
                            else if (cpu_usage > 0.6) Color.ansi(.yellow) 
                ...
                .{
                    ...
                    .label = "Memory",
                    .show_percentage = true,
                    .color = if (mem_usage > 0.9) Color.ansi(.red)
                            else if (mem_usage > 0.7) Color.ansi(.yellow)
    ...
    pub fn demoBenchmark(self: *AdaptiveCLI) !void {
        ...
        for (benchmarks) |benchmark| {
            ...
            const result = try std.fmt.allocPrint(std.heap.page_allocator, 
                "{s}: {d:.2} ms, {d:.0} ops/sec", .{ benchmark.name, duration_ms, ops_per_sec });
